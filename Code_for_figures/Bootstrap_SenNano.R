...
#Here I swap the order of the list so I don't have to change much in the remainder code
pathways <- list("SenesNano" = SenNANO, "Senes" = SenMayo)
...
## Now sample 55 genes at a time 
for (i in 1:1000) {
  sampled_genes <- sample(filtered_genes, 55, replace = FALSE)  # Sample 31 genes without replacement
...
gsva_results_df_Nano <- as.data.frame(gsva_results_transposed)

gsva_results_df_Nano[,2] <- Subset$Condition

plot_data <- data.frame(
  Value = c(gsva_results_df_Nano[, 1], average_distribution),
  Distribution = factor(c(as.character(gsva_results_df_Nano[, 2]), 
...
ggplot(plot_data, aes(x = Value, fill = Distribution)) +
  ...
  labs(title = "Distribution of Column 1 SenNano (Split by Column 2) vs. Average Distribution",
       x = "Value",
       y = "Density") +
...
ggsave("Bootstrap_SenNano.svg",plot = last_plot())
write.csv(gsva_results_df_Mayo, "gsva_results_df_Nano.csv")
